...
- (void)viewDidLoad {
    ...
    self.largeTitleLabel.text = self.cameraName;
...
- (void)addHeaderRefresh
{
    ...
    self.playbackList.header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        if (weakSelf.isSelectedDevice) {
            weakSelf.operation = [EZOpenSDK searchRecordFileFromDevice:weakSelf.cameraId
                                                         beginTime:weakSelf.beginTime
                                                           endTime:weakSelf.endTime
                                                        completion:^(NSArray *deviceRecords, NSError *error) {
                                                            NSLog(@"deviceRecords = %@, error = %@", deviceRecords, error);
                                                            if (!error)
                                                            {
        ...
        {
            weakSelf.operation =  [EZOpenSDK searchRecordFileFromCloud:weakSelf.cameraId
                                                         beginTime:weakSelf.beginTime
                                                           endTime:weakSelf.endTime
