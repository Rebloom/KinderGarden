...
- (void)viewDidAppear:(BOOL)animated {
    ...
    //修正扫描区域
    CGFloat height = self.qrView.frame.size.height;
    CGFloat width = self.qrView.frame.size.width;
    CGRect cropRect = CGRectMake((width - 240.0)/2, (height - 240.0)/3, 240.0, 240.0);
    
    [_output setRectOfInterest:CGRectMake(cropRect.origin.y/height, cropRect.origin.x/width, cropRect.size.height/height, cropRect.size.width/width)];
