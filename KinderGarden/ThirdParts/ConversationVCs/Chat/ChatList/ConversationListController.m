...
- (void)viewDidLoad {
    ...
    [headerView loadComponentsWithTitle:@"消息列表"];
    [headerView backButton];
    [headerView contactBtn];
    
    self.tableView.frame = CGRectMake(0, self.searchBar.frame.size.height+CGRectGetMaxY(headerView.frame), self.view.frame.size.width, self.view.frame.size.height - self.searchBar.frame.size.height-CGRectGetHeight(headerView.frame));
...
- (void)buttonClicked:(id)sender
{
    ...
    {
        ContactListViewController * EULVC = [[ContactListViewController alloc] init];
        [self pushToViewController:EULVC];
    }
}
...
{
    if (!_searchBar) {
        _searchBar = [[EMSearchBar alloc] initWithFrame: CGRectMake(0, CGRectGetMaxY(headerView.frame), self.view.frame.size.width, 44)];
